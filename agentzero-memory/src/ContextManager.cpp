...
double ContextManager::scoreAtomRelevance(const std::string& context_id,
                                          const Handle& atom) const
{
    std::lock_guard<std::recursive_mutex> lock(_context_mutex);

    if (!atom || context_id.empty()) {
        return 0.0;
    }
    ...
    // Membership in the context is the strongest signal
    if (entry->active_atoms.count(atom)) {
        score += 0.6;
    }

    // Shared contexts boost relevance
    auto atom_it = _atom_to_contexts.find(atom);
    if (atom_it != _atom_to_contexts.end()) {
        if (atom_it->second.count(context_id)) {
            score += 0.15;
        }
        // Overlap with other contexts of the same type
        for (const auto& other_id : atom_it->second) {
            if (other_id == context_id) continue;
            auto oit = _contexts.find(other_id);
            if (oit != _contexts.end() &&
                oit->second->context_type == entry->context_type) {
                score += 0.05;
                break;
            }
        }
    ...
    score += 0.2 * std::max(0.0, std::min(1.0, entry->importance));
...
std::vector<std::pair<Handle, double>> ContextManager::retrieveRelevantAtoms(
    const std::string& context_id,
    size_t max_results) const
{
    std::lock_guard<std::recursive_mutex> lock(_context_mutex);

    std::vector<std::pair<Handle, double>> ranked;
    auto it = _contexts.find(context_id);
    if (it == _contexts.end()) {
        return ranked;
    }

    for (const Handle& atom : it->second->active_atoms) {
        // Unlock not needed — scoreAtomRelevance locks recursively
        double s = 0.0;
        // Inline scoring to avoid double-lock issues with const method recursion
        s = 0.6 + 0.2 * std::max(0.0, std::min(1.0, it->second->importance));
        if (_active_context_id == context_id) s += 0.05;
        s = std::max(0.0, std::min(1.0, s));
        ranked.emplace_back(atom, s);
    }
    ...
    size_t max_results) const
{
    ...
    for (const Handle& atom : candidates) {
        ranked.emplace_back(atom, scoreAtomRelevance(context_id, atom));
    }
    std::sort(ranked.begin(), ranked.end(),
              [](const auto& a, const auto& b) { return a.second > b.second; });
    if (ranked.size() > max_results) {
...
std::vector<std::string> ContextManager::findRelevantContexts(
    const Handle& atom,
    size_t max_results) const
{
    std::lock_guard<std::recursive_mutex> lock(_context_mutex);
    ...
    auto atom_it = _atom_to_contexts.find(atom);
    if (atom_it != _atom_to_contexts.end()) {
        for (const auto& cid : atom_it->second) {
            auto cit = _contexts.find(cid);
            double imp = (cit != _contexts.end()) ? cit->second->importance : 0.0;
            ranked.emplace_back(cid, imp);
        }
    }
    ...
    for (size_t i = 0; i < ranked.size() && i < max_results; ++i) {
